    ...
    private func groupSchedulesByDay(schedules: [Schedule]) -> [Date: [Schedule]] {
        ...
        for schedule in schedules {
            ...
            while calendar.startOfDay(for: currentDate) <= calendar.startOfDay(for: endDate) {
                ...
                if var schedulesForDay = groupedDictionary[dayKey] {
                    schedulesForDay.append(schedule)
                    groupedDictionary[dayKey] = schedulesForDay
                } else {
                    groupedDictionary[dayKey] = [schedule]
                }
