...
public final class UserRepositoryImpl: UserRepository {
    ...
    public func fetchUser(userID: User.ID) async throws -> (User, UserDetail) {
        ...
        let userDetail = UserDetail(isFollowing: dto.isFollowing, followingCount: dto.followingCount, followerCount: dto.followerCount, totalPostCount: dto.postCount, totalCommentCount: dto.totalCommentCount, totalRoutineCount: dto.totalRoutineShareCount, isMe: dto.isMe)
    ...
    public func unBlockUser(userID: User.ID) async throws {
        try await service.requestUserUnBlock(userId: userID)
    }

    public func fetchBlockList() async throws -> [User] {
        let blockListDTO = try await service.requestUserBlockList()
        let blockList = blockListDTO.blockedUsers.map { $0.convertToEntity() }
        return blockList
    }
}
